...
def convert_examples_to_features(idx, label, code, tokenizer, args):
    add_code = ''
    del_code = ''
    for code_diff in code:  # [{'added_code': ['xx', 'xx'], 'removed_code': ['xx', 'xx']}]
    ...
    clean_code = add_code + del_code

    source_ids = tokenizer.encode(clean_code, max_length=args.block_size, padding='max_length', truncation=True)
        ...
        with open(file_path, 'rb') as f:
            ...
            for i in range(len(data)):
                self.examples.append(
                    convert_examples_to_features(i, data[i]['label'], data[i]['diff'], tokenizer, args))
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        if results['eval_acc'] > best_acc:
                            ...
                            logger.info("  Corresponding auc:%s", round(results['eval_auc'], 4))
                            logger.info("  " + "*" * 20)
...
def main():
    ...
    parser.add_argument('--adapter_name', type=str, default='apca_adapter', help="Adapter name for each layer.")
    ...
    if args.do_adapterfusion:
        adapters.init(model)
        task_adapters = ['all_codet5_pfeiffer', 'fira_codet5_pfeiffer', args.project + '_codet5_pfeiffer']
        for task_adapter in task_adapters:
            model.load_adapter(task_adapter)
        adapter_fusion = Fuse('all_codet5_pfeiffer', 'fira_codet5_pfeiffer', args.project + '_codet5_pfeiffer')
