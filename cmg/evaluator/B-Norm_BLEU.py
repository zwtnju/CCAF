...
def compute_maps(predictions, goldfile):
    predictionMap = {}
    goldMap = {}
    gf = open(goldfile, 'r')
    ...
    for row in predictions:
        ...
        predictionMap[rid] = [split_puncts(pred.strip().lower())]
    ...
    for row in gf:
        ...
        if rid in predictionMap:  # Only insert if the id exists for the method
            if rid not in goldMap:
                goldMap[rid] = []
            goldMap[rid].append(split_puncts(pred.strip().lower()))
...
if __name__ == '__main__':
    ...
    (goldMap, predictionMap) = compute_maps(predictions, reference_file)
    print(bleu_from_maps(goldMap, predictionMap)[0])
